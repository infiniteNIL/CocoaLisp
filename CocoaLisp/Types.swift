...
class CocoaLispString: DataType, Collection {
    let value: String
    ...
    var asList: List {
        return List(value.map { CocoaLispString(String($0)) })
    }

    override func copy() -> DataType {
        return CocoaLispString(value)
    }

    static func==(_ lhs: CocoaLispString, _ rhs: CocoaLispString) -> Bool {
        return lhs.value == rhs.value
    }
}
