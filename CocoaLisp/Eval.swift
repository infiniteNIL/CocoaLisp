...
func eval(_ expr: DataType, env: Environment) throws -> DataType {
    ...
    while true {
        ...
        if let first = list.elements.first as? Symbol {
            ...
            case "def":
                guard let key = list.elements[1] as? Symbol else { throw CocoaLispError.symbolExpected }
            ...
            case "defmacro", "defn":
                guard let key = list.elements[1] as? Symbol else { throw CocoaLispError.symbolExpected }
                guard let params = list.elements[2] as? Vector else { throw CocoaLispError.expectedListOfBindings }
                guard let binds = params.elements as? [Symbol] else {
                    throw CocoaLispError.expectedListOfBindings
                }
            ...
            case "fn":
                guard let params = list.elements[1] as? Vector else { throw CocoaLispError.expectedListOfBindings }
                guard let binds = params.elements as? [Symbol] else {
                    throw CocoaLispError.expectedListOfBindings
                }
            ...
            case "let":
                ...
                for index in stride(from: 0, to: bindings.count, by: 2) {
                    guard let bindingSymbol = bindings[index] as? Symbol else {
                        throw CocoaLispError.symbolExpected
                    }
            ...
            case "objc":
                guard list.elements.count >= 3 else { throw CocoaLispError.arityMismatch }
            ...
            case "try":
                ...
                do {
                ...
                catch let error as CocoaLispException {
                    if let b = b, let c = c {
                    ...
                    if let b = b, let c = c {
                        let newEnv = Environment(binds: [b], exprs: [CocoaLispString(error.message)], outer: env)
                        expr = try eval(c, env: newEnv)
                    }
                    else {
                        expr = CocoaLispString(error.message)
                    }
                }
                continue
...
private func evalAST(_ ast: DataType, env: Environment) throws -> DataType {
    ...
    case let symbol as Symbol:
        guard let lambda = env[symbol] else { throw CocoaLispError.symbolNotFound(symbol.name) }
        return lambda
