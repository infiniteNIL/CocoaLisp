...
private func add(_ args: [DataType]) throws -> DataType {
    return try args.reduce(Number(0)) { accum, arg in
        guard let number = arg as? Number else { throw CocoaLispError.invalidOperation }
        return Number(accum.value + number.value)
    }
}

private func divide(_ args: [DataType]) throws -> DataType {
    guard let first = args.first as? Number else { throw CocoaLispError.invalidOperation }
...
private func read_string(_ args: [DataType]) throws -> DataType {
    guard let s = args.first as? CocoaLispString else {
...
private func slurp(_ args: [DataType]) throws -> DataType {
    guard let filename = args.first as? CocoaLispString else {
        throw CocoaLispError.invalidOperation
    }
    let fileContents = try String(contentsOfFile: filename.value)
    return CocoaLispString(fileContents)
}
...
private func lessThan(_ args: [DataType]) throws -> DataType {
    ...
    else if let lhs = lhs as? CocoaLispString, let rhs = rhs as? CocoaLispString {
        return Boolean.eval(lhs.value < rhs.value)
    }
...
private func atom(_ args: [DataType]) throws -> DataType {
    guard args.count == 1 else { throw CocoaLispError.arityMismatch }
...
private func reset(_ args: [DataType]) throws -> DataType {
    guard args.count == 2 else { throw CocoaLispError.arityMismatch }
    guard let atom = args[0] as? Atom else {
        throw CocoaLispError.invalidOperation
    }
    atom.value = args[1]
...
private func nth(_ args: [DataType]) throws -> DataType {
    ...
    guard n.value >= 0 && n.value < elements.count else {
        throw CocoaLispError.indexOutOfRange
    }
...
private func throwException(_ args: [DataType]) throws -> DataType {
    guard args.count == 1 else { throw CocoaLispError.arityMismatch }
    throw CocoaLispException(value: args[0])
}

private func apply(_ args: [DataType]) throws -> DataType {
    guard args.count >= 2 else { throw CocoaLispError.arityMismatch }
    guard let fn = args[0] as? Function else { throw CocoaLispError.invalidOperation }
...
private func make_symbol(_ args: [DataType]) throws -> DataType {
    guard args.count == 1 else { throw CocoaLispError.arityMismatch }
    guard let name = args[0] as? CocoaLispString else { throw CocoaLispError.invalidOperation }
    return Symbol(name)
}
...
private func stringp(_ args: [DataType]) throws -> DataType {
    guard args.count == 1 else { throw CocoaLispError.arityMismatch }
    return Boolean.eval(args[0] is CocoaLispString)
}
...
private func make_hash(_ args: [DataType]) throws -> DataType {
    guard args.count % 2 == 0 else { throw CocoaLispError.arityMismatch }
    let result = HashMap()
...
private func assoc(_ args: [DataType]) throws -> DataType {
    guard args.count > 1 else { throw CocoaLispError.arityMismatch }
    guard let hash = args[0] as? HashMap else { throw CocoaLispError.invalidOperation }
...
private func readline(_ args: [DataType]) throws -> DataType {
    guard args.count == 1 else { throw CocoaLispError.arityMismatch }
    guard let prompt = args[0] as? CocoaLispString else {
        throw CocoaLispError.invalidOperation
    }
...
private func time_ms(_ args: [DataType]) throws -> DataType {
    guard args.count == 0 else { throw CocoaLispError.arityMismatch }
...
private func seq(_ args: [DataType]) throws -> DataType {
    ...
    guard let collection = args[0] as? Collection else { throw CocoaLispError.invalidOperation }
