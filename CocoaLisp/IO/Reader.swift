...
class Reader {
    ...
    static func read(_ s: String) throws -> DataType {
        ...
        if tokens.isEmpty { throw CocoaLispError.emptyData }
...
extension Reader {
    ...
    private func readDotForm() throws -> List {
        ...
        guard let dotMessage = try readForm() as? Symbol else { throw CocoaLispError.invalidOperation }
    ...
    private func readAtom() throws -> DataType {
        guard let token = next() else {
            throw CocoaLispError.unexpectedEndOfInput
        }

        if token.starts(with: "\"") {
            guard token.last == "\"" else { throw CocoaLispError.unexpectedEndOfInput }
